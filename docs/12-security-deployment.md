...
- 前端安全不是只靠后端。
- 前端负责展示、输入、存储 token、调用接口，因此也会直接暴露安全风险。
...
- Token 存储：
    ...
    - httpOnly Cookie 不能被 JS 读取，但要处理 CSRF。
    - 具体方案要结合业务风险和后端能力。

- 环境变量：
    - 前端环境变量会被打进构建产物。
    - 不能把密钥、私有 token、服务端密码放进前端环境变量。
    - 前端只能保存可以公开的配置，比如 API base URL。

- 部署：
    - 前端构建产物通常是 HTML、CSS、JS、图片等静态资源。
    - 可以部署到静态服务器、CDN、对象存储或应用服务器。
    - HTML 通常不要强缓存。
...
- 线上问题：
    ...
    - 性能监控记录加载时间、长任务、接口耗时。
    - 用户行为日志帮助复现问题。
    - Source Map 能把压缩后的线上代码映射回源码，但要控制访问权限。
