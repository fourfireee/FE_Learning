...
## 状态码是谁定的、谁展示的（容易混淆，理清三个角色）

- 谁定：状态码由后端（BE）在代码里显式决定，不是浏览器推断的。后端写 `res.status(404)` 才会有 404，没写就没有。
- 浏览器的角色：只如实把后端返回的状态码带回给前端，基本不替业务推断。
    - 例外一：根本没连上服务器（断网、域名解析失败、被 CORS 拦）时，`fetch` 直接抛异常、压根没有状态码，要用 `try/catch` 抓。
    - 例外二：请求被前置设施拦下（如 Nginx 返回 502/504、网关限流 429、CDN 返回 404），这些码不是业务后端给的，但仍是「某个服务器」给的，浏览器不会凭空编造业务码。
- 谁展示：前端（FE）读到状态码后，自己写代码决定 UX——如 401 跳登录、404 弹提示、5xx 重试或提示稍后再试。
- 给用户的具体文案怎么来：HTTP 那几十个状态码只是给程序看的粗分类，不够细分业务。具体话术（如「订单不存在」「余额不足」）和细分业务码（如 `code: "ORDER_NOT_FOUND"`）通常由后端放在响应 body 里，前端取出来展示。
- 一句话串起来：后端在代码里定状态码 → 浏览器如实传达（不推断业务码）→ 前端读到后决定怎么呈现；细分文案 / 业务码则由后端放进 body、前端取出来显示。
...
- 上传和下载本质都是用 HTTP 传二进制数据，只是方向相反；下面分别看前端怎么做。
...
- 缓存策略是否能保证发布后用户拿到新版本。
    - 通俗说：这是个常见上线事故——你发了新版本，用户打开却还是旧的。
    - 原因：浏览器会把下载过的文件（JS / CSS / 图片）缓存到本地，下次直接用本地的不再重新下载。平时省流量、加载快；但你更新网站后，用户浏览器还揣着旧文件，觉得「我本地有了」，于是看到的还是老版本（甚至带 bug 的版本）。
    - 这句话问的就是：你有没有想办法让浏览器在发新版后，知道「这个文件变了，得重新下」。
    - 通用做法（对应上面「缓存」一节）：
        - 会变的入口文件（HTML）不要强缓存，让浏览器每次都去问一下有没有更新，保证总能拿到最新的「目录」。
        - JS / CSS 用带 hash 的文件名（如 `app.3f9c.js`）：内容一改，文件名里的 hash 就变，等于变成一个「新文件」，浏览器自然会下载新的；旧文件名则可放心长期强缓存。
    - 一句话：既要用缓存加速，又要在发版后让该更新的文件确实被重新下载，别让缓存把用户卡在旧版本。

- 跨域问题是否找对了责任方（后端配置而非前端硬绕）。
    - 通俗说：遇到跨域报错，别急着在前端找偏方硬绕，真正能开口子的是后端。
    - 什么是跨域报错：你的网页用 JS 去请求另一个域名的接口，浏览器出于「同源策略」默认拦住响应、报 CORS 错。这是浏览器故意的安全保护，不是 bug。
    - 关键：这个「口子」只能由被请求的那个后端来开——它在响应里声明 `Access-Control-Allow-Origin: 你的域名`，浏览器看到才放行。这是「那台服务器愿不愿意被你访问」的问题，决定权在服务器，前端改不了。
    - 「前端硬绕」指什么、为什么不对：比如装插件关掉 CORS 检查（只对你自己电脑生效，用户照样报错）、改请求模式想骗过浏览器（要么拿不到数据、要么没真正解决）——都是治标不治本。
    - 正确做法（找对责任方）：
        - 生产环境：找后端把你的域名加进允许列表（配 CORS）。
        - 开发环境：用本地开发服务器的代理（proxy）——请求先发给同源的本地服务器，再由它转发给目标接口（服务器之间通信没有同源限制），从而绕开浏览器拦截。这是开发期的合理手段，不是硬骗浏览器。
    - 一句话：跨域是后端「要不要允许」的问题，解决方案在后端配置（或开发期用代理），而不是在前端想办法骗过浏览器。
