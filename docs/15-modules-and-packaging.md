...
- CommonJS 是 Node 早期的模块系统，现在仍大量存在于老库和 Node 脚本里。
...
- tree-shaking 指构建时把没被用到的导出代码删掉，减小体积。
...
- 特效工具和 workflow 工具都会拆成很多模块：UI 组件、坐标转换、调度器、模型客户端等。
- 清晰的模块边界让 AI 生成代码时有明确的「这段逻辑该放哪、该导出什么」的约束。
- WASM 模块（见 16 篇）也是通过 import 引入并初始化的，理解模块系统是接入 native 引擎的前提。
